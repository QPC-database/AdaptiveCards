    ...
    _In_ ABI::Windows::Foundation::Collections::IVector<ABI::AdaptiveCards::ObjectModel::Uwp::AdaptiveWarning*>* adaptiveWarnings)
{
    for (const auto& sharedWarning : sharedWarnings)
    {
        ...
        ComPtr<ABI::AdaptiveCards::ObjectModel::Uwp::IAdaptiveWarning> adaptiveWarning =
            XamlHelpers::CreateABIClass<ABI::AdaptiveCards::ObjectModel::Uwp::IAdaptiveWarning>(
                HStringReference(RuntimeClass_AdaptiveCards_ObjectModel_Uwp_AdaptiveWarning));

        RETURN_IF_FAILED(adaptiveWarning->put_StatusCode(statusCode));
        RETURN_IF_FAILED(adaptiveWarning->put_Message(warningMessage.Get()));
